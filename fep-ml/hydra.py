...
def denseNN(X_train, y_train, X_test, y_test, feature_type):
	...
	# Create the neural network with these hyper-parameters:
		...
		# for every SKOPT call, write loss to file:
		stat_output_path = "output/"+feature_type+"_skopt_convergence.csv"
		with open(stat_output_path, "a") as filepath:
			writer = csv.writer(filepath)
			writer.writerow([skopt_loss, val_loss])

			
		# Append data with best performing model.
		global startpoint_error

		if skopt_loss < startpoint_error:
			
			startpoint_error = skopt_loss
...
def trainCorrector(fitness, dimensions, n_calls, default_parameters, feature_type):
	"""
	...
	"""
	# clean slate the SKOPT convergence output file:
	stat_output_path = "output/"+feature_type+"_skopt_convergence.csv"
	if os.path.exists(stat_output_path):
		open(stat_output_path, "w").close()
	...
	optimal_params = gp_minimize(func=fitness,
						dimensions=dimensions,
						acq_func='EI', #Expected Improvement.
						n_calls=n_calls,
						x0=default_parameters,
						callback=[tqdm_skopt(total=n_calls, desc="Training")]).x

	# should we note down optimal hyperparams anywhere?
	# print("OPTIMAL HYPERPARAMS:", search_result.x)


def plotSKOPTConvergence(convergence_path):
	"""
	Given a csv file with SKOPT convergence output (generated by hydra.fitness()),
	construct an ax that plots global minima of SKOPT loss along calls.

	--args
	convergence_path (str): path to convergence file

	--returns
	ax (object): matplotlib.pyplot axis object that contains convergence plot

	"""
	convergence_data = pd.read_csv(convergence_path, 
					names=["skopt_loss", "ml_model_loss"])
	cum_min_skopt_loss = np.minimum.accumulate(convergence_data["skopt_loss"].values)

	# majority of figure complexity is added in notebook that calls this function.
	fig, ax = plt.subplots()
	ax.plot(convergence_data.index.values, cum_min_skopt_loss)
